...
- In 2025, a comprehensive dataset of 9 years of observational radar data from the WSR-88D system was created as part of the TorNet study, which used this dataset to train a CNN and compare this with other ML methods (random forests and logistic regression) and traditional forecasting methods (tornado vortex signature) using the same dataset.
...
- After initial model runs with the training data input described above, model performance was poor, and performance metrics showed that the model was mostly labeling images as “tornado no hook”. We presume one of the main causes of this is the large class imbalance between tornadoes with hooks and tornadoes without hooks (12.6% vs 87.4%, respectively).
...
- Also, picking a different radar feature that is better classified/defined to train our data on could be interesting to investigate how it affects model performance.
- Test other machine learning techniques, such as clustering or random forests, to see if these methods may be better suited to the binary image classification problem of labelling images as tornado with hook and tornado without hook. If these methods perform well, using some form of transfer learning from the baseline TorNet CNN can still be used to investigate our original second goal of tornado hook echo radar imagery to forecast tornadoes.
